...
const ActionBar: React.FC<ActionBarProps> = ({ state, dispatch, currentPlayer = 'player1' }) => {
  ...
  return (
    <div className="bg-zinc-900 p-6 border-t border-zinc-800">
       { gameMode === 'localPvp' && 
        <div className="text-center text-lg font-bold text-yellow-500 mb-4">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6 relative">
        {showEmotes && (
          <div className="absolute bottom-full left-0 mb-2 bg-zinc-800 p-2 rounded-xl border border-zinc-700 shadow-2xl flex gap-2 z-50">
            {emotes.map((e, i) => (
              <button key={i} onClick={() => handleEmote(e)} className="text-2xl hover:scale-125 transition-transform p-1">
                {e}
              </button>
            ))}
          </div>
        ...
        >
          ...
          <span className="font-bold tracking-wider text-xs">장전</span>
        </button>
        ...
        >
          ...
          <span className="font-bold tracking-wider text-xs">발사 ({bullets})</span>
          {bullets >= 5 && <span className="absolute -top-2 -right-2 bg-yellow-500 text-zinc-950 text-xs font-bold px-2 py-0.5 rounded-full animate-pulse">MAX</span>}
        </button>
        ...
        >
          ...
          <span className="font-bold tracking-wider text-xs">방어 ({blockLeft})</span>
        </button>
      </div>

      {selectedAction === 'fire' && (
        <div className="mb-6 p-5 bg-zinc-950 rounded-xl border border-zinc-800">
          <label className="block text-sm font-medium text-zinc-400 mb-4 text-center">발사할 총알 개수를 선택하세요</label>
          <input 
            type="range" 
            min="1" 
            max={bullets} 
            value={fireAmount}
            onChange={(e) => setFireAmount(parseInt(e.target.value))}
            className="w-full accent-red-500"
          />
          <div className="text-center mt-3 font-mono text-2xl text-red-500 font-bold">
            {fireAmount}
          </div>
        </div>
      )}
      ...
      <div className="flex gap-4 items-center">
          <button onClick={() => setShowEmotes(!showEmotes)} className="p-4 rounded-xl border border-zinc-800 bg-zinc-950 text-zinc-400 hover:text-zinc-100 hover:border-zinc-600 transition flex items-center justify-center">
            <MessageCircle className="w-6 h-6" />
          </button>
          <button
            onClick={submitAction}
            disabled={!selectedAction}
            className="flex-1 bg-zinc-100 text-zinc-950 font-bold rounded-xl px-4 py-4 hover:bg-zinc-300 transition-colors disabled:opacity-50 disabled:cursor-not-allowed uppercase tracking-widest"
          >
            행동 확정
          </button>
      </div>
